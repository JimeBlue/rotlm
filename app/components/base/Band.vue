<template>
  <section id="band" class="mt-16 lg:mt-24">
    <!-- Black background section -->
    <div class="bg-black pt-6 pb-6">
      <div class="container mx-auto px-4">
        <!-- Only render when data is loaded -->
        <div
          v-if="band"
          v-motion
          :initial="{ opacity: 0, y: 80 }"
          :visible="{ opacity: 1, y: 0, transition: { duration: 600, ease: 'easeOut' } }"
        >
          <h2 class="text-3xl lg:text-5xl font-bold text-white text-center uppercase">
            {{ band.title }}
          </h2>
          <div class="mx-auto mt-4 mb-8 h-1 w-20 bg-yellow-neon" />
          <div
            v-motion
            :initial="{ scale: 0.85 }"
            :visible="{
              scale: 1,
              transition: {
                type: 'spring',
                stiffness: 350,
                damping: 8,
                delay: 400,
              },
            }"
            class="max-w-3xl space-y-6 text-center mx-auto text-white origin-center"
          >
            <p class="text-lg leading-relaxed">
              {{ band.paragraph1 }}
            </p>
            <!-- Portable Text for rich text with links -->
            <PortableText
              :value="band.paragraph2"
              :components="portableTextComponents"
            />
          </div>
          <div class="mx-auto mt-8 h-1 w-20 bg-yellow-neon" />
          <!-- Band members -->
          <p v-if="band.members?.length" class="text-white text-center mt-6">
            <template v-for="(member, index) in band.members" :key="index">
              <span class="font-bold">{{ member.name }}</span> ({{ member.instrument }})<template v-if="index < band.members.length - 1">
                –
              </template>
            </template>
          </p>
        </div>

        <!-- Band photo with neon card - uses negative margin to overlap into the red band below -->
        <div
          class="relative flex justify-center items-center mt-24 lg:mt-32 mx-auto max-w-4xl mb-[-140px] lg:mb-[-200px] z-10"
        >
          <!-- Image container with green card behind -->
          <div class="relative">
            <!-- Neon green background card -->
            <motion.div
              class="absolute inset-0 w-full h-full bg-green-neon rounded-xl z-0"
              :initial="{ rotate: 0 }"
              :while-in-view="{ rotate: -6 }"
              :viewport="{ once: true, amount: 0.3 }"
              :transition="{ type: 'spring', stiffness: 150, damping: 12 }"
            />
            <!-- Band image -->
            <motion.img
              v-if="band?.bandImage"
              :src="sanityImageUrl(band.bandImage, 680)"
              :srcset="sanityImageSrcset(band.bandImage, [400, 680, 1000, 1360])"
              sizes="(min-width: 680px) 680px, 100vw"
              width="1536"
              height="864"
              loading="lazy"
              alt="ROTLM Band performing"
              class="relative w-full h-auto max-w-[680px] rounded-xl z-[1]"
              :initial="{ rotate: 0 }"
              :while-in-view="{ rotate: 4 }"
              :viewport="{ once: true, amount: 0.3 }"
              :transition="{ type: 'spring', stiffness: 150, damping: 12 }"
            />
          </div>
        </div>
      </div>
    </div>

    <!-- Red band the photo hangs into (140px/200px overlap plus generous red below it) -->
    <div class="bg-primary-500 h-64 md:h-80 lg:h-[420px]" aria-hidden="true" />

    <!-- Music: albums with Spotify embeds -->
    <div class="bg-black pb-16 lg:pb-24">
      <div class="container mx-auto px-4">
        <BaseMusicAlbums :title="home?.albumsTitle" cta />
      </div>
    </div>

    <!-- Live photos carousel, full page width -->
    <BaseHomeCarousel />

    <!-- Gigs: neon title and upcoming gigs -->
    <div class="bg-black py-16 md:py-24">
      <BaseGigsUpcoming :title="home?.gigsTitle" cta />
    </div>

    <!-- Primary-500 background section: band images -->
    <div class="bg-primary-500 pt-6 lg:pt-16 pb-0">
      <div class="container">
        <div>
          <div class="flex flex-col lg:flex-row gap-4">
            <img
              v-if="band?.imageOne"
              :src="sanityImageUrl(band.imageOne, 640)"
              :srcset="sanityImageSrcset(band.imageOne, [400, 640, 800, 1024])"
              sizes="(min-width: 1024px) 50vw, 100vw"
              :width="sanityImageDimensions(band.imageOne)?.width"
              :height="sanityImageDimensions(band.imageOne)?.height"
              loading="lazy"
              alt=""
              class="max-w-full lg:max-w-[50%] h-auto order-2 lg:order-1"
            >
            <motion.img
              v-if="band?.imageTwo"
              :src="sanityImageUrl(band.imageTwo, 640)"
              :srcset="sanityImageSrcset(band.imageTwo, [400, 640, 800, 1024])"
              sizes="(min-width: 1024px) 50vw, 100vw"
              :width="sanityImageDimensions(band.imageTwo)?.width"
              :height="sanityImageDimensions(band.imageTwo)?.height"
              loading="lazy"
              alt=""
              class="max-w-full lg:max-w-[50%] h-auto order-1 lg:order-2"
              :initial="{ rotate: 0 }"
              :while-in-view="{ rotate: [0, -3, 3, -3, 3, -2, 2, 0] }"
              :viewport="{ once: true, amount: 0.3 }"
              :transition="{ duration: 0.6, ease: 'easeInOut' }"
            />
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup>
import { PortableText } from '@portabletext/vue'
import { motion } from 'motion-v'

const { band } = useBand()
const { home } = useHome()

// Custom components for rendering Portable Text
const portableTextComponents = {
  // Render links with styling
  marks: {
    link: ({ value }, { slots }) =>
      h('a', {
        href: value?.href,
        target: '_blank',
        rel: 'noopener noreferrer',
        class: 'text-yellow-neon hover:text-white underline',
      }, slots.default?.()),
  },
  // Render paragraphs with proper styling
  block: {
    normal: (_, { slots }) => h('p', { class: 'text-white text-lg leading-relaxed' }, slots.default?.()),
  },
}
</script>
